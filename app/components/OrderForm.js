...
export default function OrderForm({ product }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const res = await fetch('/api/orders', {
        ...
        body: JSON.stringify({
          ...form,
          items: [{ sourceProductId: product.sourceProductId, quantity }],
        }),
      })
  ...
  if (status === 'success') {
    return (
      <div style={{ padding: '32px', textAlign: 'center' }}>
        ...
        <h3 style={{ fontSize: '22px', marginBottom: '8px', fontFamily: 'serif' }}>
          ההזמנה התקבלה בהצלחה!
        </h3>
        <p style={{ marginBottom: '24px', color: '#6B5C3E' }}>
        ...
        <div style={{ background: '#F8F4EE', border: '1px solid #EDE6D9', padding: '20px', marginBottom: '20px', textAlign: 'right' }}>
          ...
          <div style={{ fontSize: '24px', fontWeight: '700', color: '#8B6914', fontFamily: 'serif' }}>
            #{orderIds.ours}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: '16px', justifyContent: 'center' }}>
          <a href="/products" style={{ color: '#8B6914', textDecoration: 'none', fontWeight: '500' }}>
            ← המשך לקנות
          </a>
          <a href={`/track?order=${orderIds.ours}&email=${encodeURIComponent(form.email)}`}
            style={{ color: '#1A2332', textDecoration: 'none', fontWeight: '500' }}>
            🔍 עקוב אחרי ההזמנה
          </a>
        </div>
      </div>
  ...
  return (
    <form onSubmit={handleSubmit}>
      <h3 style={{ fontFamily: 'serif', fontSize: '20px', marginBottom: '20px', color: '#2C2416' }}>
        פרטי הזמנה
      </h3>
      ...
      <div style={{ marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <button type="button" onClick={() => setQuantity(Math.max(1, quantity - 1))}
            style={{ width: '32px', height: '32px', border: '1px solid #EDE6D9', background: '#fff', cursor: 'pointer', fontSize: '16px' }}>-</button>
